...
class Rmodal {
    ...
    document.addEventListener('click', e => {
      if (this.modal) {
        ...
        if (btnOpenModal) {
          ...
          this.currModalWindow = this.modal.querySelector(`[data-modal="${getDataBtn}"]`); // функция срабатывает один раз в определённое время, равное времени анимации

          if (new Date().getTime() - this.lastClick < this.options.speed) return;
          this.lastClick = new Date().getTime(); // модальное окно открывается
  ...
  openModal(modalWindowElem) {
    if (!this.modalOpened) {
      ...
      setTimeout(() => {
        this.modalOpened = true; // console.log('modalOpened', modalOpened)
  ...
  reOpenModal(modalWindowElem) {
    this.modalOpened = false; // console.log('modalOpened', modalOpened)
  ...
  blockScroll() {
    ...
    this.body.style.paddingRight = scrollWidth; // хак для айфонов, чтобы работал "overflow hidden" и не было прыжка страницы
  ...
  unlockScoll() {
    ...
    this.body.removeAttribute('style'); // хак для айфонов, чтобы работал "overflow hidden" и не было прыжка страницы
